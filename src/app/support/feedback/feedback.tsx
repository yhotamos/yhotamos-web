...
import type { FeedbackCategory, FeedbackEntry, ProductInfo, SiteFeedbackType, ToolFeedbackType } from "@/components/types/feedback";
...
const FEEDBACK_TYPES = {
  site: [
    { value: "typo" as SiteFeedbackType, label: "誤字・誤情報" },
    { value: "impression" as SiteFeedbackType, label: "感想" },
    { value: "other" as SiteFeedbackType, label: "その他" },
  ],
  tool: [
    { value: "bug" as ToolFeedbackType, label: "バグ報告" },
    { value: "suggestion" as ToolFeedbackType, label: "改善提案" },
    { value: "impression" as ToolFeedbackType, label: "感想" },
    { value: "other" as ToolFeedbackType, label: "その他" },
  ],
};
...
const CATEGORY_DEFAULT_TYPE: Record<FeedbackCategory, SiteFeedbackType | ToolFeedbackType> = {
  site: "typo",
  tool: "bug",
};
...
export function Feedback({ products }: { products: ProductInfo[] }) {
  ...
  const [state, setState] = useState<FormState>(() => ({
    ...INITIAL_STATE,
    ...(validToolParam && { category: "tool", toolName: validToolParam }),
  }));
  ...
  const types = FEEDBACK_TYPES[state.category];
  const isTool = state.category === "tool";
  const isBug = state.type === "bug";
  const showBugExtra = isTool && isBug;
  const showAttachment = isTool && (isBug || state.type === "suggestion");

  const onCategoryChange = (category: FeedbackCategory) => {
    setState((s) => ({ ...s, category, type: CATEGORY_DEFAULT_TYPE[category], toolName: "", browser: "", bugType: "" }));
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload: FeedbackEntry = {
        ...
        type: state.type,
        toolName: isTool ? state.toolName : undefined,
        title: state.title,
        detail: state.detail,
    ...
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      <div>
        ...
        <div className="flex gap-4 text-sm">
          {(["tool", "site"] as FeedbackCategory[]).map((c) => (
            <label key={c} className="flex items-center gap-1 cursor-pointer">
              <input type="radio" name="category" value={c} checked={state.category === c} onChange={() => onCategoryChange(c)} />
